...
export const tools: ToolPreview[] = [
  { id: "tool-1", name: "web_search", description: "Search the web for real-time information, news, and data.", usedBy: 7, icon: "globe", source: "platform", visibility: "platform", version: "2026-01-05", sideEffects: ["read_only"], inputParams: ["query", "max_results?", "date_range?"], inputExample: '{ "query": "OpenAI pricing changes 2026", "max_results": 10 }' },
  { id: "tool-2", name: "browser", description: "Navigate and interact with web pages to extract or verify information.", usedBy: 1, icon: "monitor", source: "platform", visibility: "platform", version: "2026-01-10", sideEffects: ["read_only"], inputParams: ["url", "action?", "selector?"], inputExample: '{ "url": "https://competitor.com/pricing" }' },
  { id: "tool-3", name: "code_interpreter", description: "Run code to analyze data, create charts, and process files.", usedBy: 1, icon: "terminal", source: "platform", visibility: "platform", version: "2026-02-01", sideEffects: ["read_only", "mutate_state"], inputParams: ["code", "language?", "files?"], inputExample: '{ "code": "import pandas as pd\\ndf.describe()" }' },
  { id: "tool-4", name: "pdf_reader", description: "Extract and analyze content from PDF documents.", usedBy: 3, icon: "file-text", source: "platform", visibility: "platform", version: "2026-01-15", sideEffects: ["read_only"], inputParams: ["file", "pages?", "extract_tables?"], inputExample: '{ "file": "q4-report.pdf", "pages": "1-5" }' },
  { id: "tool-5", name: "spreadsheet", description: "Read, write, and analyze spreadsheet data.", usedBy: 5, icon: "table", source: "platform", visibility: "platform", version: "2026-01-08", sideEffects: ["read_only", "write_external"], inputParams: ["file", "operation", "range?", "data?"], inputExample: '{ "file": "revenue.xlsx", "operation": "read", "range": "A1:D50" }' },
  { id: "tool-6", name: "calendar", description: "Check schedules, create events, and manage time-based triggers.", usedBy: 2, icon: "calendar", source: "platform", visibility: "platform", version: "2026-02-15", sideEffects: ["read_only", "write_external"], inputParams: ["action", "date?", "title?", "attendees?"], inputExample: '{ "action": "list", "date": "2026-03-28" }' },
  { id: "tool-7", name: "image_generation", description: "Create images and visual assets from text descriptions.", usedBy: 0, icon: "image", source: "platform", visibility: "platform", version: "2026-03-01", sideEffects: ["spend_budget"], inputParams: ["prompt", "size?", "style?"], inputExample: '{ "prompt": "Minimal logo for a fintech startup", "size": "1024x1024" }' },
  { id: "tool-8", name: "slack_send_message", description: "Send a message to a Slack channel or direct message.", usedBy: 4, icon: "send", source: "workspace", visibility: "workspace", version: "2026-03-28", sideEffects: ["send_message"], inputParams: ["channel", "text", "thread_ts?"], inputExample: '{ "channel": "#launch", "text": "Daily summary is ready." }' },
];
