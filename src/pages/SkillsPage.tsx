...
import { Search, Plus, Eye, FileText, AlertTriangle, ClipboardCheck, ShieldAlert, RefreshCw, Heart, BarChart3, PenTool, Send, Users, Package, Globe, Lock } from "lucide-react";
...
function getEmployeesUsingSkill(displayTitle: string) {
  return employees.filter(e => !e.archived && e.skills.includes(displayTitle));
}

function SourceBadge({ source, visibility }: { source: string; visibility: string }) {
  return (
...
export default function SkillsPage() {
  ...
  const filtered = skills.filter(s =>
    s.displayTitle.toLowerCase().includes(search.toLowerCase()) ||
    s.description.toLowerCase().includes(search.toLowerCase())
  );

  const selectedEntry = selected ? iconMap[selected.icon] || iconMap["eye"] : null;
  const selectedEmployees = selected ? getEmployeesUsingSkill(selected.displayTitle) : [];
  const selectedMeta = selected ? skillMeta[selected.displayTitle] || defaultMeta : defaultMeta;
    ...
    <div className="p-8 max-w-[960px] mx-auto space-y-6">
      <div className="pt-2 flex items-start justify-between opacity-0 animate-fade-in">
        <div>
          ...
          <p className="text-muted-foreground text-[13px] mt-1">Reusable capability packages your employees can use</p>
        </div>
        <button
            ...
            >
              <div className="relative">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <SourceBadge source={skill.source} visibility={skill.visibility} />
                </div>
                <h3 className="text-[14px] font-semibold text-foreground mb-1">{skill.displayTitle}</h3>
                <p className="text-[12px] text-muted-foreground leading-relaxed mb-3 line-clamp-2">{skill.description}</p>
                <div className="flex items-center justify-between">
                  <p className="text-[11px] text-muted-foreground">
                    {skill.usedBy > 0 ? `${skill.usedBy} employee${skill.usedBy > 1 ? "s" : ""} using` : "Not in use yet"}
                  </p>
                  <div className="flex items-center gap-1">
                    {skill.hasExamples && <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">examples</span>}
                    {skill.hasScripts && <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">scripts</span>}
                  </div>
                </div>
              </div>
            </div>
        ...
        <DialogContent className="sm:max-w-[480px]" onPointerDownOutside={(e) => e.preventDefault()}>
          <DialogHeader>
            ...
            <DialogDescription className="text-[13px]">
              Define a reusable capability package for your employees.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 mt-1">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">What it does / When to use</label>
              <Textarea
                value={createDescription}
                onChange={e => setCreateDescription(e.target.value)}
                placeholder="Describe what this skill does and when an employee should use it."
                className="text-[13px] min-h-[60px] resize-none"
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Instructions</label>
              <Textarea
        ...
        <DialogContent className="sm:max-w-[520px]">
            ...
            {selected && selectedEntry && (
              <div className="flex items-center gap-3 mb-1">
                ...
                <div className="flex-1 min-w-0">
                  <DialogTitle className="text-[16px]">{selected.displayTitle}</DialogTitle>
          ...
          {selected && (
            <div className="flex items-center gap-4 py-2">
              <SourceBadge source={selected.source} visibility={selected.visibility} />
          ...
          {selected && (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Package contents</p>
              </div>
              <div className="flex flex-wrap gap-1.5">
                <span className="text-[11px] px-2 py-1 rounded-md bg-muted text-foreground">instructions</span>
                {selected.hasExamples && <span className="text-[11px] px-2 py-1 rounded-md bg-muted text-foreground">examples</span>}
                {selected.hasReferences && <span className="text-[11px] px-2 py-1 rounded-md bg-muted text-foreground">references</span>}
                {selected.hasScripts && <span className="text-[11px] px-2 py-1 rounded-md bg-muted text-foreground">scripts</span>}
              </div>
              {selected.requiredTools.length > 0 && (
                <p className="text-[11px] text-muted-foreground mt-1">
                  Requires: {selected.requiredTools.join(", ")}
                </p>
              )}
            </div>
          )}
